...
### 3.2.27.3 Travel Time Indicator

| Data          | Description |
|:--------------| :--------------|
|Use Case Detail  | As a user, I want to reminded when I should leave to arrive at an event on-time.  |
|XRef           | |
|Trigger        | The Google Maps API will be used to send a reminder to the user about the time they need to leave if they expect to arrive on-time. |
|Precondition   | The user has entered a location for their event and has entered a location for their home.|
|Basic Path     | 1. The user creates an event on a specific day by clicking or tapping on the “create event.”<br />2. The user adds an address on the “Location” item of the details screen.<br>3. Once the address has been added, the app will calculate the anticipated travel time to the destination. <br />4. The event will keep track of the expected travel time and provide reminders for the user.|
|Alternate Path | None. |
|Post Condition | The user sees a reminder in the event details about their expected travel time.|
|Exception Paths| The attempt may be abandoned at any time.|
|Other          | Settings are available to adjust the "home" or point A location if the user isn't leaving from their usual location. |