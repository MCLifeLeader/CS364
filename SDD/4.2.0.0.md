...
<table>
  ...
  <tr>
    <td>event day occurence</td><td colspan = "2"> a number representing which days of the week the event is to be displayed on the calendar. Each day of the week is represented by a prime number (ex. Sunday = 1, Monday = 3, etc.) each day checked by the user adds the corrsponding day's value to get the final value, which determines the days that the event will be displayed.</td>
  </tr>
   <tr>
    <th>Methods(operations) </th>
    <th></th>
  ...
  <tr>
    <td>Returns a list of weekdays for which the event is to occur on the calendar. It does this by seperating the prime values from the total value</td>
  </tr>
   <tr>
    <th>Method Pseudo Code</th>
  </tr>
  <tr>
    ...
    primeArray = {1, 3, 7, 11, 13, 17, 19}</br>
    dayList = {}</br>
    for each number in prime array (starting at end){</br>
    if number > event day occurence:</br>
    continue</br>
    else:</br> 
    event day occurence -= number</br>
    add day array[index] to day list }</br>
    return dayList
    </td>
  </tr>
</table>
